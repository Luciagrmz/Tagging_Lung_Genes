...
def PrevWord(i, wordlist):
    if i==0:
        return("Init")
    else:
        return(wordlist[i-1])

def NextWord(i, wordlist):
    if i==len(wordlist)-1:
        return("End")
    else:
        return(wordlist[i+1])
...
def word2features(sent, i):
    ...
    features = {
        ...
        'HasHyphen':'-' in word, #
        'HasNumber':bool(re.search('\d', word)), #
        'UpperCase':bool(re.search('[A-Z]', word)), #
        'ShortAndUpperCase':ShortAndUpperCase(word), #
        'HasKeyword':word in keywords, #
        'AseEnd': bool(re.search('ases?\W*$', word)), #
        'InEnd': bool(re.search('in\W*$', word)), #
        'BNumEnd': bool(re.search('b-?\d+$', word)), #
        'InitUpper': bool(re.search("[A-Z] [a-z]*", word)), #
        'AllUpper': bool(re.search("^[A-Z]+$", word)), #
        'AlphaNumeric':bool(re.search("\w+", word)), #
        'RomanNum': bool(re.search("([ivxdlcm]+$|[IVXDLCM]+$)", word)), #
        'AllLower': bool(re.search("^[a-z]+$", word)), #
        'Letters': bool(re.search("^[A-Za-z]+$", word)), #
        'KeyWordCloseness': KeyWordCloseness(i, wordlist), #
...
if __name__ == "__main__":
    ...
    nameReport = options.trainingFile.replace('.txt', '.fStopWords_' + str(options.excludeStopWords) + '.fSymbols_' + str(
        options.excludeSymbols) + '_v3.txt')
    ...
    nameModel = options.trainingFile.replace('.txt', '.fStopWords_' + str(options.excludeStopWords) + '.fSymbols_' + str(
        options.excludeSymbols) + '_v3.mod')
